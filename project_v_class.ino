...
class Level {
    ...
    int valvePinReader;       //пин клапана 24V
...
void reciveMessage(void) {
  if (Serial.available() > 0) {
    ...
    if (String(command_in) == "GET") {
      Serial.print("001,DATA," + String(tempOne) + "," + String(vac1.vacValue) + "," + String(vlvOne.valveState) + "," + String(vlvTwo.valveState) + "," + "SBIT");
...
void loop() {
  ...
  vac1.GetVac();
  lcd.setCursor(3, 0);
  lcd.print(String(vac1.vacValue));
